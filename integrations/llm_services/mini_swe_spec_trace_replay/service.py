...
from integrations.llm_services.mini_swe_trace_replay.service import (
...
def _extract_command(content: str) -> str:
    commands = [item.strip() for item in _COMMAND_PATTERN.findall(content)]
    if len(commands) != 1:
        return ""
    return commands[0]
...
def _command_bucket(command: str) -> str:
    ...
    if _is_submission_command(normalized):
        return "submission"
    if lowered.startswith("git ") or " git " in lowered:
        return "git"
    if lowered.startswith("pytest") or lowered.startswith("tox") or " pytest " in lowered or " unittest" in lowered:
        return "test"
    if lowered.startswith("python ") or lowered.startswith("python3 ") or " python " in lowered or " python3 " in lowered:
        return "python"
    read_only_prefixes = (
        "pwd",
        "ls",
...
def _fallback_command_for_bucket(bucket: str) -> str:
    return {
        ...
        "python": "python -c \"print('draft speculative path')\"",
        "test": "python -m pytest -q",
        "read_only_shell": "pwd",
        "stateful_shell": "touch /tmp/speculative-draft",
    }.get(bucket, "pwd")
...
class TraceReplayLLMState:
    def __init__(self, *, llm_service_config: dict[str, object] | None = None) -> None:
        ...
        raw_mismatch_policy = str(config.get("mismatch_policy", "preserve_command_class")).strip().lower()
        ...
        try:
            self._response_delay_ms = max(0, int(config.get("response_delay_ms", _DEFAULT_RESPONSE_DELAY_MS)))
        ...
        try:
            self._minimal_delay_ms = max(
                0.0,
                float(_config_with_legacy_alias(config, "minimal_delay", "minimal-delay", _DEFAULT_MINIMAL_DELAY_MS)),
            )
        except (TypeError, ValueError):
        ...
        try:
            self._maximal_delay_ms = max(
                0.0,
                float(_config_with_legacy_alias(config, "maximal_delay", "maximal-delay", _DEFAULT_MAXIMAL_DELAY_MS)),
            )
        except (TypeError, ValueError):
        ...
        for index, message in enumerate(self._parsed.assistant_messages):
            bucket = _command_bucket(_extract_command(str(message.get("content", ""))))
            self._bucket_to_indices.setdefault(bucket, []).append(index)
    ...
    def restore(self, *, consumed_response_count: int) -> None:
        with self._lock:
            cursor = max(0, int(consumed_response_count))
            self._oracle_trace_cursor = cursor
            self._draft_trace_cursor = cursor
            self._pair_cache = {}
    ...
    def import_state(self, state: dict[str, Any]) -> None:
        with self._lock:
            ...
            self._draft_trace_cursor = max(0, int(state.get("draft_trace_cursor", self._oracle_trace_cursor)))
    ...
    def handle_request(self, *, path: str, headers: dict[str, str], payload: dict[str, Any]) -> dict[str, Any]:
        _ = (path, payload)
        role = str(_header_value(headers, _SPEC_ROLE_HEADER, _SPEC_ROLE_ORACLE)).strip().lower() or _SPEC_ROLE_ORACLE
        ...
        with self._lock:
            ...
            if cached is not None:
                ...
                response, effective_delay_ms = self._build_response_locked(role=role, pair_id=pair_id)
    ...
    def _build_response_locked(self, *, role: str, pair_id: str) -> tuple[dict[str, Any], float]:
        if role == _SPEC_ROLE_ORACLE:
            if self._oracle_trace_cursor >= len(self._parsed.assistant_messages):
                raise RuntimeError("mini_swe_spec_trace_replay exhausted all oracle assistant responses")
        ...
        if self._draft_trace_cursor >= len(self._parsed.assistant_messages):
            raise RuntimeError("mini_swe_spec_trace_replay exhausted all draft assistant responses")
        ...
        command = _extract_command(str(original_message.get("content", "")))
        accepted = True
        if not _is_submission_command(command):
            accepted = self._draft_decision(trace_index=trace_index, pair_id=pair_id)
        if accepted:
            self._accept_count += 1
        else:
            self._reject_count += 1
            message["content"] = self._mutate_content(
    ...
    def _mutate_content(self, *, original_content: str, original_command: str, trace_index: int) -> str:
        replacement = self._replacement_command(original_command=original_command, trace_index=trace_index)
        match = _COMMAND_PATTERN.search(original_content)
        if match is None:
            return f"THOUGHT: try an alternative command\n\n<mswea_bash_command>{replacement}</mswea_bash_command>"
        start, end = match.span(1)
        return f"{original_content[:start]}{replacement}{original_content[end:]}"

    def _replacement_command(self, *, original_command: str, trace_index: int) -> str:
        ...
        for candidate_index in self._bucket_to_indices.get(bucket, []):
            if candidate_index == trace_index:
                continue
            candidate_command = _extract_command(str(self._parsed.assistant_messages[candidate_index].get("content", "")))
