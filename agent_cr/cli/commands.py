"""Argparse tree + output formatting for the `agentcr` CLI."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="agentcr",
        description=(
            "Operator CLI for the Agent-CR daemon. The daemon owns all runc / "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Daemon Unix socket path. Defaults to $AGENT_CR_DAEMON_SOCKET or "
    ...
    daemon = sub.add_parser(
        "daemon",
        help="Start, stop, or inspect the Agent-CR daemon process.",
    )
    daemon_sub = daemon.add_subparsers(dest="daemon_command", required=True)

    p_start = daemon_sub.add_parser(
        "start",
        help="Start the Agent-CR daemon in the background. Like `dockerd`, "
    ...
    # at the top level so the verbs read cleanly (`agentcr checkpoint ls`,
    # `agentcr checkpoint rm ...`) and so future cross-sandbox operations
    # (`agentcr checkpoint prune --orphaned`, etc.) fit naturally.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    logging.basicConfig(
        level=os.environ.get("AGENT_CR_CLI_LOG_LEVEL", "WARNING").upper(),
    ...
    try:
        ...
        print(f"agentcr: {exc}", file=sys.stderr)
        return 2
    except DaemonRequestError as exc:
        print(f"agentcr: daemon returned {exc.status_code}: {exc.body.decode('utf-8', 'replace')}", file=sys.stderr)
        return 3
...
def _cmd_daemon_start(args: argparse.Namespace) -> int:
    ...
    if client.ping():
        print(f"agentcr daemon already running at {socket_path}", file=sys.stderr)
        return 0
    cmd: list[str] = [
        sys.executable,
        "-m",
    ...
    if log_file is None:
        log_file = socket_path.parent / "agentcr-daemon.log"
    ...
    while time.monotonic() < deadline:
        if proc.poll() is not None:
            print(
                f"agentcr daemon exited during startup (rc={proc.returncode}); "
                f"see {log_file} for details",
                file=sys.stderr,
            )
            return 1
        if client.ping():
            print(f"agentcr daemon started (pid={proc.pid}, socket={socket_path})")
            print(f"logs: {log_file}")
            return 0
        time.sleep(0.1)
    print(
        f"agentcr daemon did not become ready within {args.timeout:.1f}s; "
        f"see {log_file}",
        file=sys.stderr,
    )
    return 1
...
def _cmd_daemon_stop(args: argparse.Namespace) -> int:
    ...
    client = DaemonClient(socket_path, timeout_seconds=args.timeout)
    if not client.ping():
        print(f"agentcr daemon not running at {socket_path}", file=sys.stderr)
        return 0
    # Ask politely first.
    try:
    ...
    while time.monotonic() < deadline:
        if not client.ping():
            print(f"agentcr daemon stopped (socket={socket_path})")
            return 0
        time.sleep(0.1)
    print(
        f"agentcr daemon did not stop within {args.stop_timeout:.1f}s; "
...
def _cmd_sandbox_rm(args: argparse.Namespace) -> int:
    ...
    for sid in args.sandbox_ids:
        try:
            ...
            print(f"agentcr sandbox rm {sid}: {exc}", file=sys.stderr)
            rc = 1
    return rc
...
def _cmd_sandbox_run(args: argparse.Namespace) -> int:
    ...
    # Import lazily — pulling agent_cr.Sandbox at module import time would
    # drag the engine surface into every CLI invocation, including
    # `agentcr daemon start` where the daemon's own Engine.start path
    ...
    for assignment in args.env or []:
        if "=" not in assignment:
            print(f"agentcr sandbox run: --env requires KEY=VALUE, got {assignment!r}", file=sys.stderr)
            return 2
        key, value = assignment.split("=", 1)
        env[key.strip()] = value

    socket_path = _resolve_socket(args)
    os.environ["AGENT_CR_DAEMON_SOCKET"] = str(socket_path)
    ...
    try:
        ...
        if args.detach or not args.argv:
            ...
            return 0
        # Strip a leading "--" so users can write `agentcr sandbox run IMAGE -- cmd ...`.
...
def _cmd_restore(args: argparse.Namespace) -> int:
    """`agentcr restore SANDBOX_ID CHECKPOINT_ID`.
...
def _cmd_sandbox_exec(args: argparse.Namespace) -> int:
    if not args.argv:
        print("agentcr sandbox exec: need a command after `--`", file=sys.stderr)
        return 2
    socket_path = _resolve_socket(args)
