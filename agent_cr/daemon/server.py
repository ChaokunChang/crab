"""Agent-CR daemon process — the long-running host service.

The daemon owns exactly one in-process Engine (`agent_cr.engine.Engine`)
and exposes it over a Unix-socket HTTP API. Clients (the SDK proxy in
`agent_cr.remote_engine.RemoteEngine` and the `agentcr` CLI) translate
...
Running model:
  ...
  - State persistence across daemon restarts is deferred. A v1 restart
    ...
    state is the operator's responsibility (`agentcr sandbox rm` is
...
def _build_handler(daemon: "DaemonServer"):
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version = "agentcr-daemon/1"
        sys_version = ""

        def log_message(self, fmt: str, *args: Any) -> None:
            logger.debug("agentcr-daemon: " + fmt, *args)
...
class DaemonServer:
    ...
    def start(self) -> None:
        ...
        logger.info("Starting Agent-CR daemon; socket=%s", self._socket_path)
        ...
        logger.info(
            "Agent-CR daemon ready: socket=%s perms=%o pid=%d sandbox_count=%d",
            self._socket_path,
            DEFAULT_SOCKET_PERMS,
            os.getpid(),
            0,
...
# ---------------------------------------------------------------------------
# CLI entry point — used by `python -m agent_cr.daemon` and the
# `agentcrd` console script.
# ---------------------------------------------------------------------------
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="agentcrd",
        description="Run the Agent-CR daemon (long-running host service).",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=None,
        help="Path to EngineConfig YAML. Defaults to $AGENT_CR_DAEMON_CONFIG or built-in defaults.",
    )
    parser.add_argument(
        "--socket",
        type=Path,
    ...
    parser.add_argument(
        "--log-level",
        default=os.environ.get("AGENT_CR_DAEMON_LOG_LEVEL", "INFO"),
    ...
    if config_arg is None:
        env_config = os.environ.get("AGENT_CR_DAEMON_CONFIG")
        if env_config:
