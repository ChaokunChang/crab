"""Agent-CR daemon — the long-running host service that owns the Engine.

Operators start the daemon once per host (`python -m agent_cr.daemon`) and
...
Public surface:
  ...
  - `DaemonClient` (transport.py): used by both the SDK proxy and the
    `agentcr` CLI to talk to the daemon.
  - `default_socket_path()` (transport.py): the conventional path the
    daemon listens on (`$XDG_RUNTIME_DIR/agentcr/agentcr.sock` for users,
    `/run/agentcr/agentcr.sock` for root).
"""
