...
(`agent_cr.remote_engine.RemoteEngine`), and a single `serve_unix_socket`
...
DEFAULT_SOCKET_NAME = "agentcr.sock"
...
def default_socket_path() -> Path:
    ...
    Honors `$AGENT_CR_DAEMON_SOCKET` first, then falls back to
    `$XDG_RUNTIME_DIR/agentcr/agentcr.sock` for non-root users and
    `/run/agentcr/agentcr.sock` for root. Mirroring the dockerd convention
    keeps the discovery rule predictable for both the CLI and the SDK
    proxy."""
    explicit = os.environ.get("AGENT_CR_DAEMON_SOCKET")
    if explicit:
        return Path(explicit).expanduser().resolve()
    if os.geteuid() == 0:
        base = Path("/run/agentcr")
    else:
        xdg = os.environ.get("XDG_RUNTIME_DIR")
        base = Path(xdg).expanduser() if xdg else Path.home() / ".cache" / "agentcr"
        base = base / "agentcr"
...
class DaemonRequestError(RuntimeError):
    ...
    def __init__(self, status_code: int, path: str, body: bytes) -> None:
        ...
        try:
            ...
            decoded = repr(body)
        super().__init__(
            f"agentcr daemon request failed: status={self.status_code} path={self.path} body={decoded}"
        )
...
class _UnixHTTPConnection(http.client.HTTPConnection):
    ...
    def __init__(self, socket_path: str, *, timeout: float) -> None:
        ...
        super().__init__("agentcr", timeout=timeout)
        ...
        *,
        ...
        try:
            ...
            raise FileNotFoundError(
                f"agentcr daemon not reachable at {self._socket_path} ({exc.__class__.__name__}); "
                "start the daemon with `agentcr daemon start` (or "
                "`python -m agent_cr.daemon`)."
            ) from exc
        finally:
            conn.close()
        if status_code >= 400:
