"""SDK-side proxy that makes a remote Agent-CR daemon look like a local Engine.
...
class RemoteEngine:
    ...
    def stop(self) -> None:
        ...
        clears the local sandbox registry. Use `agentcr daemon stop`
    ...
    def client(self) -> DaemonClient:
        """Underlying daemon client. Useful for tests and the `agentcr` CLI."""
        return self._client
