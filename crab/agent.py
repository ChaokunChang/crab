...
agent into an Crab sandbox. It intentionally has a narrower contract than
...
# Provider environment variables Crab sets while an agent is running, or
    ...
    def command_env(self, overrides: dict[str, str] | None = None) -> dict[str, str]:
        ...
        if base_url:
            for var in llm_env_vars_for(self.llm_protocol):
                env[var] = base_url
            env["CRAB_SANDBOX_ID"] = str(self.sandbox.sandbox_id)
        if overrides:
    ...
    def run(self, task: str) -> TaskResult:
        ...
        try:
            if base_url:
                ...
                prior_env["CRAB_SANDBOX_ID"] = os.environ.get("CRAB_SANDBOX_ID")
                os.environ["CRAB_SANDBOX_ID"] = str(sbx.sandbox_id)
