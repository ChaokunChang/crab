...
- root
- Docker
- build deps for `crab/host_inspector/bpf`
...
```bash
make -C crab/host_inspector/bpf
```

## Start The Server

```bash
python3 -m crab.host_inspector --port 9782 --log-level INFO
```
...
```bash
python3 -m crab.host_inspector.watch --base-url "$HOST_INSPECTOR_URL" "$SANDBOX_ID"
```
...
```bash
CRAB_RUN_PERF=1 python3 -m unittest \
