...
| `incremental_process_enabled` | `bool` | `False` | Opt-in CRIU incremental process dumps. When `True`, every chain participant becomes a `runc checkpoint --pre-dump` followed by a `runc checkpoint --parent-path` so non-anchor checkpoints write only delta memory pages. Requires `RuntimeCapabilities.supports_incremental_process=True`; falls back to a single full dump with a logged warning otherwise. Applies to every scheduler policy. | `_resolve_incremental_process()` in `scheduler.py`, `AdapterProcessCWorker.checkpoint()` in `workers/process.py` |
| `full_process_checkpoint_interval` | `int` | `8` | Policy knob. Reset the chain after this many checkpoints (anchor counted as #1). The next checkpoint becomes a fresh anchor whose pre-dump has no parent. Must be ≥ 1. Counts only successful process-bearing checkpoints; skipped, filesystem-only, and failed checkpoints don't advance the counter. | `_resolve_incremental_process()` in `scheduler.py`, `InMemorySchedulerStateStore.record_process_checkpoint()` in `scheduler.py` |
| `max_process_chain_length` | `int` | `16` | Safety cap. Whichever of `full_process_checkpoint_interval` or this triggers first forces a fresh anchor. In normal operation the interval trips first and this is dead code; it is load-bearing only when the in-memory chain-length counter mis-tracks (e.g., a missed `record_process_checkpoint`, a future bug that bypasses the interval check). Must be ≥ 1. Bound it small enough to keep restore-time chain walks short. | `_resolve_incremental_process()` in `scheduler.py` |
...
| `enable_fork_chain_sharing` | `bool` | `false` | **Phase B fork-prep optimization.** When `true`, fork prep replaces per-fork `shutil.copytree` of every chain ancestor's CRIU image with relative symlinks via `Runtime.link_ancestor_pre_dump` and `Storage.link_ancestor_artifact`. The leaf's `pre_dump/parent` symlink is preserved (`copytree(symlinks=True)`); the planted ancestor symlinks resolve into the source's bytes. `Storage.pin_chain` / `unpin_chain` extends the existing pin refcount so `LatestOnlyCheckpointManager` can't prune ancestors while a fork holds them. On promote, `Runtime.materialize_linked_pre_dumps` and `Storage.materialize_linked_artifacts` inline the borrowed bytes before the source is destroyed. Requires `incremental_process_enabled=true` to have ancestors to share. **Recommended default-on**: cleanest single optimization — ~−40 % mean `fork_restore_ms` AND ~−3 % wall-clock on the terminus 8-task subset, with zero observed cost. The chain pin also fixes a latent retention correctness issue (without it, retention can prune a chain ancestor while a fork holds a symlink into it). |
| `enable_background_prefork` | `bool` | `false` | **Phase A fork-prep optimization.** When `true`, `_SpeculativeSandboxController` warms a fork after each successful checkpoint via a dedicated executor. Hooked centrally by wrapping `scheduler.mark_checkpoint_complete`, so auto-mode and manual checkpoint paths both fire. `ensure_fork()` validates `_cached_fork_checkpoint_id` against the latest, drops stale prefits, and optionally waits briefly for an in-flight prefork. Without throttling, background warming saturates disk I/O and starves active workloads (observed 23 s prefork outlier on `rstan-to-pystan`); pair with `prefork_max_concurrent_global` and `prefork_min_interval_seconds`. **Opt-in only**: even with the throttle, A's I/O contention is intrinsic — a single-sandbox smoke (`rstan_smoke`) costs +322 s (+17.4 %) wall-clock for a 4.8 s saving on the agent-loop wait. Use when per-turn `fork_restore_ms` jitter matters more than aggregate wall-clock (interactive use, latency-sensitive measurements); skip when the workload is I/O-bound or wall-clock throughput is the optimization target. |
| `prefork_min_interval_seconds` | `float` | `0.0` | Per-sandbox debounce on `kick_prefork`. Skips kicks that arrive within this window of the previous one. Recommended `2.0` when `enable_background_prefork=true`; the default un-throttled `0.0` is fine for tests where preforks are rare. |
| `prefork_wait_timeout_seconds` | `float` | `0.0` | When `ensure_fork()` is called and a background prefork is in flight, wait up to this many seconds for it to finish before falling through to the synchronous-clone path. Recommended `0.05` for spec scenarios — enough for the prefork to land in cache when the agent's oracle round-trip is starting, short enough to not block the agent perceptibly. |
| `prefork_max_concurrent_global` | `int` | `0` | **Required for stable throughput when `enable_background_prefork=true`.** Class-level bounded semaphore caps concurrent in-flight preforks across ALL active sandboxes. `0` disables the throttle (every sandbox can prefork concurrently — this saturates disk I/O on multi-sandbox runs). Recommended `2` for 8-sandbox spec runs: cuts `fork_restore_ms` max from 23 s to 1.8 s while still allowing two sandboxes to warm in parallel. Note: the throttle eliminates outliers but does NOT eliminate steady-state I/O contention with active sandbox work — see `enable_background_prefork` for the opt-in caveat. Telemetry counter: per-controller `_prefork_skipped_throttled`. |
| `enable_lazy_restore` | `bool` | `false` | **Phase D fork-prep optimization.** When `true`, `RuncRestoreOptions.lazy_pages` makes `runc restore` return once metadata + a small eager set is mapped. Pages stream in via userfaultfd from a `criu lazy-pages -D <image> -W <work>` daemon (spawned ahead of `runc restore`; socket existence is the readiness signal; idempotent SIGTERM→SIGKILL reaper handles cleanup). Requires kernel `unprivileged_userfaultfd=1` or root + `CAP_SYS_PTRACE`, runc 1.3+ with `--lazy-pages` support, and CRIU 4.x. **Recommended default-on for spec scenarios when the platform supports it**: ~−19 % mean `fork_restore_ms` at ~−1.5 % wall-clock, no observed downside. |
